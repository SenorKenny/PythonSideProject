...
def json_parser(json_file,schema,purchased):
    ...
    try:
        if json_file is None:
            logger.debug("Something went wrong with requester.")
            return None,boolean_dictionary
    except Exception as error:
            logger.exception(f"Some error unbeknownst to me: {error}")
    ...
    for x in json_file:
        ...
        if price == 0:
            logger.debug(f"unable to obtain price for {x}")
        if price>too_high:
        ...
        try:
            ...
            logger.warning(f"Title unable to be retrieved for {x} ")
        ...
        if name in schema:
            if "watching" in schema[name]["status"]:
                 if price != schema[name]["price"]:
                      ...
                      logger.info("modifying schema, changing info on a watched product")
            else:
                 continue
        else:
            try:
                ...
                logger.warning("couldn't retrieve category")
            ...
            logger.info("Adding new item to schema")
    return boolean_dictionary